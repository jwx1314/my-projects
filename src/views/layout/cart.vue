  ...
  <div class="cart">
    ...
    <div v-if="isloading && cartlist.length > 0">
      ...
      <div class="footer-fixed">
        <div class="all-check" @click="allchangechecked(!isallchecked)">
        ...
        <div class="all-total">
          <div class="price">
            <span>合计：</span>
            <span
              >¥ <i class="totalPrice">{{ selcartsum }}</i></span
            >
          </div>
          <div
            v-if="!isEdit"
            class="goPay"
            :class="{ disabled: selcartnum === 0 }"
            @click="handlepay()"
          >
            结算({{ selcartnum }})
          </div>
...
export default {
  ...
  methods: {
    ...
    allchangechecked (flag) {
      this.$store.commit('cart/allchangecheck', flag)
    },
    changecount (value, goodsid, goodsskuid) {
      this.$store.dispatch('cart/changecount', { value, goodsid, goodsskuid })
    },
    clearpro () {
      if (this.Countsum === 0) return
      this.$store.dispatch('cart/clearlistpro')
      this.isEdit = false
    },
    handlepay () {
  ...
  watch: {
    isEdit (value) {
      if (value) {
        this.$store.commit('cart/allchangecheck', !value)
      } else {
        this.$store.commit('cart/allchangecheck', !value)
      }
    }
